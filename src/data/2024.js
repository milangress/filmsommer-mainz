export const year = 2024

const dates = [
	{
		date: '2024-08-26',
		name: 'Eröffnungsabend',
		events: [
			{
				time: '18:00:00',
				type: 'Live-Musik',
				title: 'King Mofa',
				fskHidden: false,
				content:
					'King Mofa aus Mainz spielen Reggae, Calypso, Neue Deutsche Welle, ein bisschen Punkrock und etwas Funk. Ein mitreißender Mix aus Sommer und handgemachter Musik, die einfach gute Laune macht.'
			},
			{
				time: '19:00:00',
				type: 'Eröffnung',
				title: 'Offizielle Eröffnung',
				fskHidden: false,
				content:
					'Mit dem Schirmherrn Markus Müller sowie den Festivalmacher*innen Tidi von Tiedemann und Yvonne Wuttke.'
			},
			{
				time: '20:30:00',
				type: 'Spielfilm',
				title: 'Franky Five Star',
				fsk: '12',
				content: `"Sei einfach du selbst!" Wer kann das schon? Franky jedenfalls hat vier Stimmen in ihrem Kopf, die ihr Leben immer wieder durcheinanderbringen. In ihrer Kopfwelt wohnt sie mit Ella, Frank, Lenny und Frau Franke in einem alten Hotel. Während Franky einfach nur ihr Leben in den Griff kriegen will, haben ihre Alter Egos ganz eigene Ziele. Als sie ausgerechnet dem neuen Freund ihrer besten Freundin immer näherkommt, bricht in ihrem Kopf-Hotel Chaos aus.<br><br>
        <b>Regie</b> Birgit Möller <br>
        <b>Produktion</b> Achtung Panda! <br>
        <b>Ko-Produktion</b> Aamu Film Company, ZDF/ Das kleine Fernsehspiel <br>
        <b>Gefördert von</b> BKM, MFG, nordmedia, DFFF, Finnish Film Foundation <br>
        <b>Verleih</b> jip-film.de/franky-five-star/ <br>
        <b>Erscheinungsjahr</b> Deutschland, Finnland 2022 <br>
        <b>FSK</b> ab 12 <br>
        <b>115 Min</b><br><br>
        <b>Mit Unterstützung: ZDF/Das kleine Fernsehspiel</b>`,
				pathText: 'Tragikomödie'
			}
		],
		image: 'FRANKY_FIVE_STAR_01.png',
		imageAlt:
			'Film still aus Franky Five Star. Franky Zusammen mit ihrem Nachbar vor einem verschwommenen romantischen Hintergrund (Jahrmarkt)',
		imageText: 'Franky Five Star'
	},
	{
		id: 2,
		date: '2024-08-27',
		name: 'Kurz- und Mittellanger Film des FILMZ-Festivals',
		events: [
			{
				time: '20:30:00',
				type: 'Filme',
				title: 'Best of FILMZ',
				fsk: '12',
				content: `Als erstes Langfilmfestival in RLP präsentiert FILMZ seit dem Jahr 2001 alljährlich aktuelle, deutschsprachige Produktionen aus Deutschland, Österreich und der Schweiz. Neben dem Langfilm- und Kurzfilmwettbewerb veranstaltet FILMZ seit 2008 und 2010 auch Wettbewerbe in den Sektionen Mittellanger Film und Dokumentarfilm.<br><br>
          <b>Die Telefonzelle</b> • Deutschland 2023 • FSK 12 • 11 Min.<br>
          <b>Mach's Licht aus</b> • Deutschland 2022 • FSK 6 • 11 Min.<br>
          <b>I was never really here</b> • Deutschland 2022 • FSK 6 • 23 Min.<br>
          <b>Lost and found</b> • Deutschland 2023 • FSK 12 • 18 Min.<br>
          <b>Auf Sand gebaut</b> • Deutschland 2023 • FSK 6 • 30 Min.<br><br>
          <b>Kooperation: FILMZ Festival des deutschen Kinos</b>`,
				pathText: 'Mittellange- & Kurzfilme'
			}
		],
		image: [
			{
				url: 'Auf Sand gebaut_still.png',
				imageText: 'Auf Sand gebaut'
			},
			{
				url: 'I_was_never_really_here.png',
				imageText: 'I was never really here'
			},
			{
				url: 'Lost_and_found.png',
				imageText: 'Lost and found'
			},
			{
				url: 'Machs_Licht_aus.png',
				imageText: 'Machs Licht aus'
			},
			{
				url: 'Die_Telefonzelle.png',
				imageText: 'Die Telefonzelle'
			}
		]
	},
	{
		id: 3,
		date: '2024-08-28',
		name: 'Sommerempfang der Kreativforen Rheinland-Pfalz',
		events: [
			{
				time: '18:00:00',
				type: 'Einlass',
				title: 'Schorle-Empfang',
				content: ''
			},
			{
				time: '19:00:00',
				type: 'Begrüßung & Talk',
				title: '',
				title_long: 'Begrüßung & Kreativwirtschaft RLP: Impulse für die Demokratie',
				content: `Begrüßung durch Wirtschaftsministerin RLP Daniela Schmitt. <br><br>
        Moderatorin Eva Schmitt und Vertreter*innen der Branchen Design, Film, Medien, Games und Musik über Projekte zum Thema Demokratie.`
			},
			{
				time: '20:30:00',
				type: 'RLP-Shorts',
				pathText: 'Kurzfilme & Trailer',
				title: 'Kurzfilme & Trailer',
				title_long: 'RLP-Shorts: Kurzfilme & Trailer von Medien-Talents aus Rheinland-Pfalz',
				content: `Die Branchen Design, Games, Film und Medien sind herzlich eingeladen zum Sommerempfang der Kreativforen RLP 2024. Das Designforum Rheinland-Pfalz, Game Up! Gamesforum Rheinland-Pfalz und das Film- und Medienforum Rheinland-Pfalz laden im Rahmen des Filmsommer Festival Mainz in Kooperation mit Kultur & Vergnügen zum Talk mit Vertreter*innen der Branchen und anschließendem Netzwerken unter freiem Himmel.
          <br><br>
          Anmeldung: <a href="http://www.designforum-rlp.de">www.designforum-rlp.de</a>`
			}
		],
		image: 'Kreativforen_RLP.png',
		imageAlt: ''
	},
	{
		id: 4,
		date: '2024-08-29',
		name: 'Mainzer Stummfilm-Abend',
		events: [
			{
				time: '20:30:00',
				type: 'Stummfilm & Musik',
				title: 'Das historische Mainz',
				fsk: '0',
				content: `Szenen aus 100 Jahren Mainzer Alltag. Bilder der Stadt, ihrer Feste und Feiern, vom Sport, von der Kultur, vom Leben. Aus rund fünfzig privaten Filmeinsendungen von Leser*innen der Allgemeinen Zeitung Mainz hat das Archiv der Stadt Mainz die schönsten ausgesucht, digitalisiert, geschnitten und zu einer Chronik der anderen Art verbunden.<br><br>
          <b>Regie</b> Privatfilmaufnahmen <br>
          <b>Musikalische Live-Untermalung</b> Jazzband Trio Mayence <br>
          <b>Verleih</b> STADTMEDIA – MAINZ/AZ Mainz <br>
          <b>FSK</b> 0 <br>
          <b>60 Min</b><br><br>
          <b>Kooperation: Initiative Zitadelle Mainz e. V, Stadthistorisches Museum</b>`,
				pathText: 'Stummfilm'
			},
			{
				time: '20:30:00',
				type: 'Live-Musik',
				title: 'Trio Mayence',
				content:
					'Jazz, Swing & Bossa – die Jazzband mit Stil. Sympathisch, stilvoll, elegant und unaufdringlich in der Untermalung vertont die Band live und ganz individuell unseren Stummfilm.'
			}
		],
		image: 'Das_historische_Mainz.png',
		imageAlt:
			'Schwarzweiß Film still aus Das historische Mainz. Straßenbild mit auto und marktplatz',
		imageText: 'Das historische Mainz'
	},
	{
		id: 5,
		date: '2024-08-30',
		name: 'Generation Now – Jugendfilmabend',
		events: [
			{
				time: '20:30:00',
				type: 'Jugendfilm-Drama',
				title: 'GEORGIE',
				fsk: '6',
				content: `<i>Originaltitel: Scrapper (englisch für „Schläger“) ist ein britisches Jugendfilm-Drama.</i><br><br>
          ES BRAUCHT EIN GANZES DORF, UM EIN KIND ZU ERZIEHEN? VON WEGEN! GEORGIE ERZIEHT SICH GANZ ALLEIN!<br><br>
          Georgie braucht niemanden, schon gar keinen Erwachsenen, der ihr sagt, was sie tun oder lassen soll. Seit dem Tod ihrer Mutter lebt die Zwölfjährige allein in der Sozialbau­wohnung in London und wimmelt trickreich Anrufe besorgter Sozialarbeiterinnen ab. Mit Diebstählen verdient sie ein wenig Geld. Nur manchmal ist Georgie traurig, wenn sie sich eine alte Sprachnachricht ihrer Mutter anhört. Dann steht eines Tages ein fremder Mann vor der Tür und behauptet, ihr Vater zu sein. Ist er ein Gangster? Oder gar ein Vampir? Langsam lässt sie sich auf Jason ein, der manchmal viel mehr Kind ist, als sie selbst. Sommerlich leicht und voller Humor erzählt der Film von der Annäherung der beiden und dem gemeinsamen Erwachsen­ und wieder Kindwerden.<br><br>
          <b>Buch & Regie</b> Charlotte Regan <br>
          <b>FSK</b> 6 <br>
          <b>83 Min</b> <br>
          <b>OmU</b><br><br>
          <b>2023</b> <i>Großer Preis der Jury</i> für den besten ausländischen Spielfilm beim Sundance Film Festival<br>
          <b>2023</b> <i>European Young Audience Award</i> bei den European Film Awards<br><br>
          <b>Kooperation: LUCAS – Internationales Festival für junge Filmfans</b>`,
				pathText: 'Jugendfilm-Drama'
			}
		],
		image: 'GEORGIE.png',
		imageAlt:
			'Film still aus Georgie. Ein Teenager und ein junges mädchen zeigen aufeinander vor einem mit pflanzen überwachsenen Platz.',
		imageText: 'Georgie'
	},
	{
		id: 6,
		date: '2024-08-31',
		name: 'Politischer Filmabend, Preisverleihung & Abschlussparty',
		events: [
			{
				time: '18:00:00',
				type: 'Live-Musik',
				title: 'Absinto Orkestra',
				content:
					'EINEWELTMUSIK – eine Band zwischen Balkan-Beat und Gipsy-Swing, tanzbar, melancholisch und vor allem handgemacht.'
			},
			{
				time: '20:00:00',
				type: 'Preisverleihung',
				title: 'Albert Treber Preis',
				content: `Wir freuen uns, zum zweiten Mal die Gewinner des Albert Treber Preises im Rahmen des Filmsommer Festivals Mainz bekannt zu geben. Dieser Preis ehrt Personen oder Institutionen für ihren herausragenden Einsatz und ihr Engagement im Bereich des Filmschaffens.<br><br>
          Albert Treber, nach dem der Preis benannt ist, war ein engagierter Mainzer Filmförderer und Unterstützer von Nachwuchsfilmemacher*innen. Sein Beitrag legte den Grundstein für die Filmförderung in Rheinland-Pfalz. Der Preis wurde geschaffen, um das Erbe von Albert Treber zu würdigen und die besondere Leidenschaft und den herausragenden Beitrag der Preisträger*innen zur Filmkunst anzuerkennen.<br><br>
          Der Albert Treber Preis wird vom medien . rlp – Institut für Medien und Pädagogik e. V. gestiftet und von dem Künstler Johannes Grünthal gestaltet.<br><br>
          Die fachkundige Jury wird darüber entscheiden, an welche Person oder Institution der Preis verliehen wird. Wir sind gespannt auf die diesjährige Auswahl und freuen uns auf die feierliche Preisverleihung beim Filmsommer Festival Mainz.<br><br>
          <b>Jurymitglieder</b><br>
          <ul>
          <li><b>Ursula Simgen-Buch</b> vom Provinz Programmkino Enkenbach (Preisträgerin des letzten Jahres)</li>
          <li><b>Stephan Falk</b> (Autor, Grimme-Preisträger, Gewinner des Deutschen Filmpreises Lola)</li>
          <li><b>Uli Holzhausen</b> (Musiker, Aktivist und Grün- dungsmitglied des Open Ohr Festivals)</li>
          <li><b>Mario von Wantoch-Rekowski</b> (medien.rlp – Institut für Medien und Pädagogik e.V., Geschäftsführer von medien.rlp)</li>
          </ul>
          <br>
          <b>Kooperation: medien . rlp – Institut für Medien und Pädagogik e.V., Rosa Luxemburg Stiftung RLP</b>`
			},
			{
				time: '20:30:00',
				type: 'Mystery-Thriller',
				title: 'Im toten Winkel',
				fsk: '16',
				content: `In einer abgelegenen Stadt im Nordosten der Türkei versucht ein deutsches Filmteam, einen Dokumentarfilm zu drehen. Derweil setzt sich ein türkischer Überwachungsagent mit scheinbar mystischen Kräften auseinander, die seine kleine Tochter verfolgen. Während sich die Geschichten verflechten, entfaltet sich ein komplexes Netz der Verschwörung.<br><br>
          <b>Buch & Regie</b> Ayşe Polat <br>
          <b>Erscheinungsjahr</b> Deutschland 2023 <br>
          <b>FSK</b> 16 <br>
          <b>117 Min</b> <br>
          <b>OmU</b><br><br>
          <b>2024</b> Deutscher Filmpreis`,
				pathText: 'Mystery-Thriller'
			},
			{
				time: '22:00:00',
				type: 'Jubiläumsparty',
				title: 'mit DJ Janeck – Oriental Tropical',
				content:
					'Im Bereich globale Rhythmen prägt er seit vielen Jahren das Rhein-Main-Gebiet mit Projekten wie <i>La Bolschevita, Oriental Tropical</i> und <i>Mach-Mal-Langsam</i>.'
			}
		],
		image: 'Im_toten_Winkel.png',
		imageAlt:
			'Film still aus Im toten Winkel. Zwei Frauen blicken besorgt auf einen Mann der nur am unteren rand des Bildes zu sehen ist.',
		imageText: 'Im toten Winkel'
	}
]

const about = {
	text: `<p>Liebe Freund*innen der Filmkultur unter freiem Himmel, liebe Besucher*innen des Filmsommer Festivals Mainz,</p>
<p>wir freuen uns, euch auch dieses Jahr wieder zu einem besonderen Festival einladen zu können, das nun schon zum 11. Mal stattfindet.</p>
<p>Vom 26. bis zum 31.08.2024 erwartet euch wieder ein buntes und vielseitiges Programm – und unsere mittlerweile traditionelle Spielstätte, die Kulturei an der Mainzer Zitadelle, bleibt dabei der zentrale Veranstaltungsort.</p>
<p>Freut euch auf wunderbare Sommerabende voller inspirierender Filme, Livemusik und spannender Begegnungen. Von Bildern vom historischen Mainz über das Sommerfest der Kreativforen bis hin zum politischen Filmabend erwartet euch ein spannendes und abwechslungsreiches Programm. Schaut euch auf den folgenden Seiten um und lasst euch von den zahlreichen Höhepunkten dieses Jahres mitreißen.</p>
Beste Grüße und „Film ab!“<br>
Euer Filmsommer-Team<br>
<br>
<p>
<b>Veranstaltungsort:</b><br>
<address>
Zitadelle Mainz<br>
Kulturei<br>
Zitadellenweg, 55131 Mainz<br>
</address>
</p>`
}

export { about, dates }
