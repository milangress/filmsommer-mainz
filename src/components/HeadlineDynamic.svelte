...
<div class="headline-dynamic {className}" bind:this={headlineRef} id="{id}">
    <div class="top-row">
        {#each topRow as item}
            <div class="top-row--item" style="transform: translate({Math.random() * 50}%, {item * 50}%)"></div>
        {/each}
    </div>
    <div class="top-row">
        {#each topRow as item}
    ...
    <div class="left-row">
        {#each leftRow as item}
            <div class="left-row--item" style="transform: translate({item * 50}%, {Math.random() * 50}%)"></div>
        {/each}
    </div>
    <div class="left-row">
        {#each leftRow as item}
    ...
    <div class="bottom-row">
        {#each bottomRow as item}
            <div class="bottom-row--item" style="transform: translate({-Math.random() * 50}%, {item * 50}%)"></div>
        {/each}
    </div>
    <div class="bottom-row">
        {#each bottomRow as item}
